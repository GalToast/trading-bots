...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Churn lattice lab: open opposite .01 tickets as price extends, "
            "close only winning recovered bags, and keep the residual lattice open. "
            "Reports realized bank separately from leftover floating exposure."
        )
    )
    parser.add_argument("--symbols", nargs="*", default=DEFAULT_SYMBOLS)
    parser.add_argument("--days", type=int, default=20)
    parser.add_argument("--step-pips", type=float, default=1.0)
    parser.add_argument("--bank-pips", type=float, default=0.5)
    parser.add_argument("--anchor-reset-pips", type=float, default=3.0)
    parser.add_argument("--max-open-per-side", type=int, default=50)
    parser.add_argument(
        "--output-csv",
        default=str(ROOT / "reports" / "churn_lattice_lab.csv"),
    )
...
def favorable_pips(direction: str, entry_price: float, current_price: float, pip_size: float) -> float:
    move = (current_price - entry_price) / pip_size
...
def simulate_symbol(symbol: str, bars: list[dict], symbol_info, cfg: Config) -> dict:
    ...
    bank_px = cfg.bank_pips * pip_size
    ...
    for idx in range(1, len(bars)):
        ...
        for ticket in open_tickets:
            threshold_price = ticket.entry_price + bank_px if ticket.direction == "BUY" else ticket.entry_price - bank_px
            hit = bar["high"] >= threshold_price if ticket.direction == "BUY" else bar["low"] <= threshold_price
            if hit:
                realized_pnls.append(unit_pnl_usd(symbol, ticket.direction, ticket.entry_price, threshold_price, spread_px))
            else:
                survivors.append(ticket)
        open_tickets = survivors
