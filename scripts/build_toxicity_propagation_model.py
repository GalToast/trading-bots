#!/usr/bin/env python3
"""
Neural Harpoon Phase 1: Toxicity Propagation Model
Trains an XGBoost model to predict if a Coinbase microstructure toxicity event
will propagate into a Kraken warp flush/surge within 30 seconds.
"""
...
MODEL_OUTPUT = os.path.join(MODELS_DIR, "kraken_toxicity_harpoon_v1.joblib")
...
def main():
    ...
    features = ["price", "reload_multiple", "vol_24h", "size_delta_pct", "mag_level"] + list(action_cols)
    X = df[features]
    y = df["target"]
    ...
    # We might have only one class in y_test if data is small, handle gracefully
    try:
