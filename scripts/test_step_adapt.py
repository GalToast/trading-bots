#!/usr/bin/env python3
"""Test: my engine with dynamic step adaptation (matching StatefulRearmRawEngine)."""
...
def run_engine(bars, info, step, max_open, gap, alpha, momentum_gate, use_adapt=False):
...
print(f"  {'Step Mode':<16} {'Combined':>12} {'Closes':>7} {'Rearm':>6} {'MaxSeen':>7}")
print(f"  {'-'*50}")

for use_adapt in [False, True]:
    r = run_engine(bars, info, 15.0, 80, 1, 1.0, False, use_adapt)
    label = "Fixed" if not use_adapt else "Adaptive"
