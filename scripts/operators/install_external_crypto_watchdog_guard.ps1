...
$ExternalScriptPath = Join-Path $SupervisorDir "watch_crypto_watchdog.ps1"
$LauncherPath = Join-Path $SupervisorDir "run_crypto_watchdog_guard_hidden.vbs"
$TaskName = "TradingBots-CryptoWatchdog-Guard"
$TaskDescription = "External one-shot guard outside the repo that restarts the MT5 crypto watchdog loop if its heartbeat or process goes stale."
...
`$LoopStatePath = Join-Path `$RepoRoot 'reports\watchdog\crypto_watchdog_loop_state.json'
`$EnsureScriptPath = Join-Path `$RepoRoot 'scripts\operators\ensure_crypto_watchdog.ps1'
`$RepoAlertPath = Join-Path `$RepoRoot 'reports\watchdog\crypto_watchdog_alerts.jsonl'
...
`$LogPath = Join-Path `$LogDir 'crypto_watchdog_guard.log'
`$GuardStatePath = Join-Path `$LogDir 'crypto_watchdog_guard_state.json'
`$LocalAlertPath = Join-Path `$LogDir 'crypto_watchdog_alerts.jsonl'
...
function Post-SwitchboardAlert([string]`$Message) {
    ...
    try {
        & `$PythonExe `$PostSwitchboardScript '@crypto-watchdog-guard' `$Message | Out-Null
    }
    catch {
...
function Emit-Alert([string]`$Severity, [string]`$EventType, [string]`$Reason, [int]`$ProcessCount, [double]`$AgeSeconds, [object]`$LoopState) {
    ...
    `$title = ('TradingBots crypto watchdog ' + `$EventType)
    `$message = ('severity=' + `$Severity + '; reason=' + `$Reason + '; process_count=' + `$ProcessCount + '; loop_age_s=' + ([math]::Round(`$AgeSeconds, 1)))
    `$payload = @{
        ts_utc = `$nowText
        severity = `$Severity
    ...
    Post-SwitchboardAlert ('[crypto-watchdog-guard] ' + `$EventType + ' | ' + `$message)
