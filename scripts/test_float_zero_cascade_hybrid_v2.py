...
def run_hybrid_controller(symbol: str, bars: list, cfg: dict) -> SymbolState:
    ...
    spread_px = spread_price(info) if info else 180.0
    base_step = cfg.get("base_step", 15.0)
    max_open = cfg.get("max_open", 60)
    ...
    for idx in range(1, len(bars)):
        ...
        if controller == "ema_ribbon":
            ...
            if compressed: step = max(base_step * 0.5, 0.01)
        ...
        current_float = 0.0
        for t in tickets:
            # We use bar close for floating check
        ...
        # Bar extreme cascade close
        # Only close if it's profitable at the bar extreme
        to_remove = []
        sl = sorted([t for t in tickets if t.direction == "SELL"], key=lambda t: t.entry_price, reverse=True)
        # Check if the shallowest sell is profitable
...
def main():
    ...
    print(f"FLOAT ZERO CASCADE HYBRID V2")
    print()
        ...
        for step in [15.0, 25.0, 50.0]:
            for hf in [0, 1, 2]:
                for fz in [True, False]:
                    configs.append({
                        "label": f"{mode} step={step:.0f} hf={hf} fz={fz}",
    ...
    for cfg in configs:
        ...
        results.append((cfg["label"], {
            ...
            "max_open": state.max_open_total, "mae": state.max_adverse_excursion,
