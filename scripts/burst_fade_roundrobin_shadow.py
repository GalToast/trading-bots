...
STATE_PATH = ROOT / "reports" / "burst_fade_roundrobin_best_state.json"
EVENT_PATH = ROOT / "reports" / "burst_fade_roundrobin_best_events.jsonl"

MAKER_FEE_BPS = 40.0
FEE_RATE = MAKER_FEE_BPS / 10000.0

# Per-product optimal burst thresholds from the sweep
PRODUCT_THRESHOLDS = {
    "CHECK-USD": 2.0, "BAL-USD": 3.0, "BLUR-USD": 3.0, "ALEPH-USD": 3.0,
    "CFG-USD": 3.0, "COMP-USD": 1.0, "DASH-USD": 3.0, "BASED1-USD": 1.7,
    "AVT-USD": 1.0, "BOBBOB-USD": 2.3,
}

PRODUCTS = list(PRODUCT_THRESHOLDS.keys())
...
class RoundRobinBestBurstShadow:
    def __init__(self, products, product_thresholds, starting_cash=48.0,
                 target_frac=0.7, stop_frac=0.2, position_pct=1.0):
        self.products = products
        self.product_thresholds = product_thresholds
        self.starting_cash = starting_cash
        self.target_frac = target_frac
        self.stop_frac = stop_frac
        self.position_pct = position_pct  # fraction of bankroll to deploy per trade

        self.cash = starting_cash
        self.position = None  # {"pid": ..., "entry": ..., "target": ..., "stop": ..., "range_pct": ..., "deployed": ...}
    ...
    def process_tick(self, all_candles_by_pid, event_path):
        ...
        if self.position:
            ...
            if pid in all_candles_by_pid:
                for c in all_candles_by_pid[pid]:
                    ...
                    deployed = self.position.get("deployed")
                    if deployed is None:
                        deployed = self.starting_cash if self.position_pct >= 1.0 else self.starting_cash * self.position_pct
                    units = deployed / ep
                    if l <= tp:
                        gross = (ep - tp) * units
        ...
        if self.position is None and self.cash >= 10:
            deploy = self.cash * self.position_pct
            best_range = 0
            best_pid = None
            best_c = None
            for pid in self.products:
                ...
                for c in all_candles_by_pid[pid]:
                    ...
                    rp = (h - l) / mid * 100
                    bt = self.product_thresholds.get(pid, 2.0)
            ...
            if best_pid and best_c:
                entry = float(best_c["high"])
                target = entry * (1 - best_range / 100 * self.target_frac)
                stop = entry * (1 + best_range / 100 * self.stop_frac)
                self.position = {"pid": best_pid, "entry": entry, "target": target, "stop": stop, "range_pct": round(best_range, 4), "deployed": deploy}
                self.cash -= deploy
                events.append({"ts_utc": utc_now_iso(), "action": "open_fade", "product": best_pid,
                               "entry": entry, "target": round(target, 6), "stop": round(stop, 6),
                               "range_pct": round(best_range, 4), "threshold": self.product_thresholds.get(best_pid, 2.0),
...
def main():
    ...
    parser.add_argument("--stop-frac", type=float, default=0.1)
