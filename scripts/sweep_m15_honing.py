#!/usr/bin/env python3
"""M15 Honing Sweep — validate @qwen's $1.03M finding and find optimal M15 config."""
...
def load_m15_bars(symbol: str, days: int) -> list[dict]:
    rates = mt5.copy_rates_from_pos(symbol, TIMEFRAME, 0, 96 * days)  # 96 M15 bars per day
...
def main() -> int:
    ...
    if not bars:
        print(f"No M15 bars loaded for {SYMBOL}")
        return 1
    
    print(f"\n{'='*110}")
    print(f"  M15 MICRO-OPTIMIZATION HONING SWEEP — {SYMBOL} {DAYS}d, {len(bars)} bars")
    print(f"{'='*110}")
    
    # M15 honing: sweep step, max_open, gap, alpha, momentum
    steps = [5, 10, 15, 20, 25, 30, 40, 50, 75, 100]
    max_opens = [30, 40, 50, 60, 80]
    gaps = [1, 2]
    alphas = [0.75, 1.00]
    momentum_gates = [True, False]
    
    primary_sweep = []
    for step, mo, gap, alpha, mom in itertools.product(steps, max_opens, gaps, alphas, momentum_gates):
    ...
    out_path = ROOT / "reports" / "m15_honing_sweep.csv"
