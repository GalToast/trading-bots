#!/usr/bin/env python3
"""
RAVE Crown Jewel Live Shadow
Asset: RAVE-USD
Logic: RSI(4) < 30 entry, 25% TP, 3% SL, 24-bar timeout, RSI(4) > 80 exit.
...
STATE_PATH = ROOT / "reports" / "rave_crown_jewel_state.json"
EVENT_PATH = ROOT / "reports" / "rave_crown_jewel_events.jsonl"
        ...
        if self.position:
            if m5_candles:
                for c in m5_candles:
                    ...
                    elif rsi >= OB_THRESH or self.position["hold"] >= MAX_HOLD:
                    ...
                    if closed:
                        ...
                        events.append({"ts_utc": utc_now_iso(), "action": "close", "exit": exit_p, "net": round(net, 4)})
                        self.position = None
                        break
...
def main():
    client = CoinbaseAdvancedClient(); engine = RaveCrownJewelShadow()
