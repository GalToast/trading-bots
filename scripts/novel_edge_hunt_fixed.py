"""Novel Edge Hunt — 10 strategies × 5 coins, bugs FIXED."""
...
def strat_opening_range():
    """FIX: Track first 12 bars of each UTC day regardless of hour alignment."""
    ...
    def fn(i, candles, ts, op, hi, lo, close, v, btc_lk):
        ...
        if state["day"] != day:
            state["day"] = day
            state["range_high"] = 0
            state["range_low"] = float("inf")
            state["bars"] = 0
        # First 12 bars of each day = building range
        if state["bars"] < 12:
            state["range_high"] = max(state["range_high"], hi)
            state["range_low"] = min(state["range_low"], lo)
            state["bars"] += 1
            return None
        # After building phase, trade breakouts
        if state["bars"] >= 12 and state["range_high"] > 0:
...
def strat_fibonacci():
    def fn(i, candles, ts, op, hi, lo, close, v, btc_lk):
        if i < 20: return None
        recent_highs = [float(candles[j]["high"]) for j in range(max(0,i-20), i)]
        recent_lows = [float(candles[j]["low"]) for j in range(max(0,i-20), i)]
        if not recent_highs or not recent_lows: return None
...
def strat_relative_strength():
    """FIX: Less restrictive — coin outperforming BTC when BTC is weak."""
    def fn(i, candles, ts, op, hi, lo, close, v, btc_lk):
        if i < 10: return None
        pt, pt3 = ts - 60, ts - 180
        if pt not in btc_lk or pt3 not in btc_lk: return None
        btc_mom = (btc_lk[pt] - btc_lk[pt3]) / btc_lk[pt3]
        # Coin above its short-term avg while BTC is flat/down
        if btc_mom < 0.001:
            avg_5 = sum(float(candles[j]["close"]) for j in range(max(0,i-5), i)) / min(5, i)
            if close > avg_5 * 1.005:
                return {"fill": op, "tp": op * 1.10, "sl": op * 0.95, "max_hold": 20}
        return None
    return fn

def strat_multi_tf_rsi():
    """FIX: Properly average 3 M5 bars to simulate 1 M15 candle."""
    def fn(i, candles, ts, op, hi, lo, close, v, btc_lk):
        if i < 15: return None
        # M5 RSI(3)
        ...
        # M15: average 3 consecutive M5 bars per M15 candle, then RSI(3)
        m15_closes = []
        for k in range(5):  # 5 M15 candles = 15 M5 bars
            start_j = i - 15 + k * 3
            end_j = start_j + 3
            if start_j >= 0 and end_j <= i:
                avg_close = sum(float(candles[j]["close"]) for j in range(start_j, end_j)) / 3
                m15_closes.append(avg_close)
        if len(m15_closes) < 3: return None
        rsi_m15 = rsi(m15_closes, 3)
...
def main():
    ...
    for coin in COINS:
        print(f"\n{'='*80}")
        print(f"📊 {coin} (30d) — 10 novel strategies (BUG-FIXED)")
        print(f"{'='*80}")
        try:
    ...
    for strat_name in strategies:
        ...
        print(f"\n  {strat_name}:")
        print(f"    Total: ${total_net:.2f}, {total_trades}t | Avg WR: {avg_wr:.1f}%, DD: {avg_mdd:.1f}%, PF: {avg_pf:.2f}")
        print(f"    Best: {best['coin']} (${best['net']:.2f}, {best['trades']}t, {best['wr']}% WR)")

    profitable = [r for r in all_results if r["net"] > 30]
    if profitable:
        print(f"\n{'='*80}")
        print(f"🚀 STRATEGIES CLEARING $30/30d")
        print(f"{'='*80}")
    ...
    with open("reports/novel_edge_hunt_fixed.json", "w") as f:
        json.dump(all_results, f, indent=2)
    print(f"\nSaved to reports/novel_edge_hunt_fixed.json")
