#!/usr/bin/env python3
"""30d reconciliation: TROLL, SUP, MDT through strategy library."""
import sys, os
...
coins = ['TROLL-USD', 'SUP-USD', 'MDT-USD']
lines = []
...
for coin in coins:
    ...
    try:
            ...
            for tp in [5, 10, 15]:
                for sl in [0, 3, 5]:
                    ...
                    if r['net_pnl'] > 0 and r['win_rate'] >= 40:
                        qualifying += 1
                        if qualifying <= 15:  # Only show first 15 qualifying combos
        ...
        if best:
            ...
            lines.append(f"  Hit rate: {hit_rate:.1f}% ({qualifying}/{total} combos profitable)")
            if r['net_pnl'] > 0 and r['win_rate'] >= 40:
                lines.append(f"  [CONFIRMED 30d EDGE]")
            else:
                lines.append(f"  [30d FAILED]")
        else:
...
result = "\n".join(lines)
with open("reports/reconciliation_troll_sup_mdt.txt", "w", encoding="utf-8") as f:
    f.write(result)
