...
DEFAULT_STATE_PATH = ROOT / "reports" / "kraken_spot_machinegun_shadow_state.json"
DEFAULT_EVENT_PATH = ROOT / "reports" / "kraken_spot_machinegun_shadow_events.jsonl"
DEFAULT_OPPORTUNITY_TAPE_PATH = ROOT / "reports" / "kraken_spot_machinegun_opportunity_tape.jsonl"
STRATEGY_BOARD_PATH = ROOT / "reports" / "kraken_spot_frontier_strategy_board.json"
BEAR_VELOCITY_PATH = ROOT / "reports" / "kraken_spot_money_velocity_board.json"
MFE_TRACKER_PATH = ROOT / "reports" / "kraken_spot_machinegun_mfe_tracker.json"
...
class MachinegunShadowEngine:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            "mode": "kraken_spot_machinegun_shadow",
    ...
    def maybe_close_positions(self, ticks: dict[str, dict[str, Any]], *, event_path: Path) -> None:
        for product_id in list(self.active_positions.keys()):
            ...
            if float(mark["bid"]) <= float(mark["trail_stop"]) and float(mark["net_pnl"]) >= self.min_profit_to_trail_usd:
...
def fetch_kraken_ticks(client: KrakenSpotClient, product_ids: list[str]) -> dict[str, dict[str, Any]]:
    ...
    for product_id, data in payload.items():
        ...
        # Kraken ticker keys can be aliases, we rely on the caller to handle mapping, or just use the key.
        # But we need to make sure we map it back to the requested product_id if it doesn't match perfectly.
        # A simple hack is just return what was queried, but payload.items() returns the alias.
        # The best way is to iterate over product_ids and see if an alias matches or just find the one that ends with the requested quote.
    ...
    # Since Kraken returns aliases (e.g. XXBTZUSD instead of BTCUSD), we map back to the requested product_ids.
    mapped_ticks = {}
    for pid in product_ids:
        # Simplistic mapping: check if the exact ID exists, or if any alias ends with the base+quote (without dash).
        clean_pid = pid.replace("-", "")
        if pid in ticks:
            mapped_ticks[pid] = ticks[pid]
        elif clean_pid in ticks:
            mapped_ticks[pid] = ticks[clean_pid]
        else:
            # Fallback: just find the first returned tick (if we only queried one) or match roughly
...
def run_once(
    ...
    *,
    ...
    candidate_ticks = fetch_kraken_ticks(client, tracked_products) if tracked_products else {}
    ...
    for product_id in list(engine.active_positions.keys()):
        ...
        tick = candidate_ticks.get(product_id) or fetch_kraken_tick(client, product_id)
        ...
        if decision.get("decision") == "rotate_to_challenger":
            ...
            if challenger:
                challenger_tick = candidate_ticks.get(challenger_id) or fetch_kraken_tick(client, challenger_id)
    ...
    while len(engine.active_positions) < max_pos:
        ...
        if eligible:
            ...
            decisions.append(decision)
            tick = candidate_ticks.get(top_id) or fetch_kraken_tick(client, top_id)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--require-ml-survival-prob", type=float, default=0.0)
    parser.add_argument("--require-fast-green-prob", type=float, default=0.0)
    parser.add_argument("--require-tail-prob", type=float, default=0.0)
...
def main() -> int:
    ...
    engine.fee_source = "static_kraken"
    engine.fee_tier = "pro"
