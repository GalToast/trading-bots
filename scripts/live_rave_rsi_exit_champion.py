#!/usr/bin/env python3
"""
RAVE RSI Exit Champion Live Shadow
Asset: RAVE-USD
Logic: RSI(4) < 30 entry, RSI(4) > 80 EXIT (primary), 25% TP, 7% SL, 24-bar timeout.
Includes: BTC M1 Momentum Gate + Session Gate for safety.

NEW CHAMPION: RSI exit at 80 captures mean reversion reversals.
Backtest: $422.64/72h (880.5%), 232 trades, 67.2% WR.
Previous crown jewel: $225.63/72h (470.1%), 64 trades, 43.8% WR.
"""
import argparse
import json
import os
...
STATE_PATH = ROOT / "reports" / "rave_rsi_exit_champion_state.json"
EVENT_PATH = ROOT / "reports" / "rave_rsi_exit_champion_events.jsonl"
...
RSI_PERIOD = 4
OS_THRESH = 30
OB_THRESH = 80  # Exit when RSI > 80
TP_PCT = 25.0
SL_PCT = 7.0  # Wider stop (NoSL was $311, 7% is $240 — compromise)
MAX_HOLD = 24
...
class RaveRsiExitChampionShadow:
    def __init__(self, starting_cash=48.0):
        self.starting_cash = starting_cash
        self.cash = starting_cash
        self.position = None  # {"entry": ..., "target": ..., "stop": ..., "quote": ..., "hold": 0}
        ...
        if self.position:
            if m5_candles:
                for c in m5_candles:
                    ...
                    # RSI exit at 80 (PRIMARY — this is the champion edge)
                    if rsi >= OB_THRESH:
                        exit_p = cl
                        exit_reason = "rsi_ob"
                    elif h >= self.position["target"]:
...
def main():
    ...
    engine = RaveRsiExitChampionShadow()
    ...
    print(f"Backfilling 72h data for {PRODUCT} RSI Exit Champion...", flush=True)
    try:
