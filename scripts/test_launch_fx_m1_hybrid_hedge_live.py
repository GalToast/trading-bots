...
import launch_fx_m1_hybrid_hedge_live as launcher


class LaunchFxM1HybridHedgeLiveTests(unittest.TestCase):
    def test_build_lane_contract_matches_gbp_hybrid_margin_surviving_row(self) -> None:
        lane = launcher.build_lane_contract("GBPUSD")
        args = lane["restart_args"]

        self.assertEqual(lane["name"], "live_gbpusd_m1_snake_hybrid_941797")
        ...
        self.assertEqual(args[args.index("--hedge-mode") + 1], "depth_threshold")
        ...
        self.assertEqual(args[args.index("--live-magic") + 1], "941797")
        self.assertEqual(
            args[args.index("--variant-label") + 1],
            "snake_step0.1pip_retrace1_hold0_static_float_zero_hedgedepth_threshold4_cap16_rebase",
        )
        self.assertAlmostEqual(lane["contract_meta"]["winner_booked_usd_per_hour"], 3.35, places=3)
    ...
    def test_build_lane_contract_matches_eur_hybrid_margin_surviving_row(self) -> None:
        lane = launcher.build_lane_contract("EURUSD")
        args = lane["restart_args"]

        self.assertEqual(lane["name"], "live_eurusd_m1_snake_hybrid_941798")
        ...
        self.assertIn("--positive-only-closes", args)
        self.assertIn("--block-on-prestart-open-carry", args)
        self.assertEqual(args[args.index("--live-magic") + 1], "941798")
        self.assertEqual(
            args[args.index("--variant-label") + 1],
            "snake_step0.1pip_retrace1_hold0_static_float_zero_hedgesame_level_cap16_fixed",
        )
        self.assertAlmostEqual(lane["contract_meta"]["max_entry_spread_ratio"], 14.0, places=3)
        self.assertAlmostEqual(lane["contract_meta"]["winner_booked_usd_per_hour"], 0.976, places=3)
        self.assertTrue(lane["contract_meta"]["positive_only_closes"])
        self.assertTrue(lane["contract_meta"]["block_on_prestart_open_carry"])
        self.assertFalse(lane["contract_meta"]["rebase_on_flat"])
        self.assertNotIn("--rebase-on-flat", args)

    def test_ensure_watchdog_membership_updates_fx_lists_once(self) -> None:
        watchdog = {"groups": {"fx_watchdog": {"lanes": ["live_old"]}}}
        changed_first = launcher.ensure_watchdog_membership(watchdog, "live_gbpusd_m1_snake_hybrid_941797")
        changed_second = launcher.ensure_watchdog_membership(watchdog, "live_gbpusd_m1_snake_hybrid_941797")
        self.assertTrue(changed_first)
        self.assertFalse(changed_second)
        self.assertIn("live_gbpusd_m1_snake_hybrid_941797", watchdog["groups"]["fx_watchdog"]["lanes"])
        self.assertIn("live_gbpusd_m1_snake_hybrid_941797", watchdog["fx_watchdog"]["lanes"])
    ...
    def test_build_lane_contract_includes_rebase_flag_when_spec_enables_it(self) -> None:
        lane = launcher.build_lane_contract("GBPUSD")
        self.assertIn("--rebase-on-flat", lane["restart_args"])
        self.assertTrue(lane["contract_meta"]["rebase_on_flat"])

    def test_build_lane_contract_supports_single_symbol_magic_override(self) -> None:
        lane = launcher.build_lane_contract("GBPUSD", live_magic=951797)
        self.assertEqual(lane["name"], "live_gbpusd_m1_snake_hybrid_951797")
        self.assertEqual(lane["state_path"], "reports/live_gbpusd_m1_snake_hybrid_951797_state.json")
        self.assertEqual(lane["event_path"], "reports/live_gbpusd_m1_snake_hybrid_951797_events.jsonl")
        self.assertEqual(lane["restart_args"][lane["restart_args"].index("--live-magic") + 1], "951797")
        self.assertEqual(lane["contract_meta"]["live_magic"], 951797)

    def test_deactivate_other_family_rows_pauses_superseded_contracts(self) -> None:
        registry = {
            "lanes": [
                {"name": "live_gbpusd_m1_snake_hybrid_941797", "enabled": True, "pause_note": ""},
                {"name": "live_gbpusd_m1_snake_hybrid_951797", "enabled": True, "pause_note": ""},
                {"name": "live_eurusd_m1_snake_hybrid_941798", "enabled": True, "pause_note": ""},
            ]
        }
        watchdog = {
            "groups": {
                "fx_watchdog": {
                    "lanes": [
                        "live_gbpusd_m1_snake_hybrid_941797",
                        "live_gbpusd_m1_snake_hybrid_951797",
                        "live_eurusd_m1_snake_hybrid_941798",
                    ]
                }
            },
            "fx_watchdog": {
        ...
        changed = launcher.deactivate_other_family_rows(
            registry,
            watchdog,
            family_prefix="live_gbpusd_m1_snake_hybrid_",
            keep_lane_name="live_gbpusd_m1_snake_hybrid_951797",
        )
        self.assertTrue(changed)
        old_row = registry["lanes"][0]
        kept_row = registry["lanes"][1]
        self.assertFalse(old_row["enabled"])
        self.assertEqual(old_row["pause_note"], "superseded_by_live_gbpusd_m1_snake_hybrid_951797")
        self.assertTrue(kept_row["enabled"])
        self.assertEqual(
            watchdog["groups"]["fx_watchdog"]["lanes"],
            ["live_gbpusd_m1_snake_hybrid_951797", "live_eurusd_m1_snake_hybrid_941798"],
        )
        self.assertEqual(
    ...
    def test_cutover_requires_superseded_family_rows_to_be_broker_flat(self) -> None:
        registry = {
            "lanes": [
                {
                    "name": "live_gbpusd_m1_snake_hybrid_941797",
                    "restart_args": ["scripts/live_snake_counter_web_shadow.py", "--live-magic", "941797"],
                },
                {
                    "name": "live_gbpusd_m1_snake_hybrid_951797",
                    "restart_args": ["scripts/live_snake_counter_web_shadow.py", "--live-magic", "951797"],
                },
            ]
        }
        with (
