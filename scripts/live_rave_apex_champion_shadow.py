#!/usr/bin/env python3
"""
THE APEX CHAMPION (Long-Only)
Logic: The ultimate synthesis of structural, leading, and regime data.
1. Regime Scaler: 
   - Vol > 3.0%: Deploy 95%
   - Vol 1.5-3%: Deploy 25%
   - Vol < 1.5%: STAY IN CASH
2. Lattice Gate: BTC M1 Momentum > -0.1%
3. Entry: RSI(4) < 45 
4. Confirm: OB Ratio > 2.0 AND Ratio Velocity > 0
...
STATE_PATH = ROOT / "reports" / "apex_champion_state.json"
EVENT_PATH = ROOT / "reports" / "apex_champion_events.jsonl"
...
class ApexChampionShadow:
    ...
    def process_tick(self, client, m5_candles, btc_tick, event_path):
        ...
        vol = compute_volatility(self.history[-20:]) if len(self.history) >= 20 else 0.0
        
        # Volatility Scaler
        deploy_fraction = 0.0
        if vol >= 0.03: deploy_fraction = 0.95 # Pump
        elif vol >= 0.015: deploy_fraction = 0.25 # Active
        else: deploy_fraction = 0.0 # Dead

        dt_now = datetime.now(timezone.utc)
        session_gate = (dt_now.hour not in [12, 19, 6, 0])

        if self.position is None and self.cash >= 10.0 and btc_gate and deploy_fraction > 0 and session_gate:
            ...
            if rsi_now <= OS_ENTRY:
                ...
                try:
                    ...
                    if ratio >= 2.0 and velocity > 0:
                        ...
                        tq = self.cash * deploy_fraction
                        if tq < 10.0: tq = 10.0
                        if tq > self.cash: return events
                        
                        self.position = {"pid": PRODUCT, "entry": ep, "quote": tq, "hold": 0}
                        self.cash -= tq
                        events.append({"ts_utc": utc_now_iso(), "action": "open", "vol": round(vol, 4), "fraction": deploy_fraction, "ratio": round(ratio, 2)})
                except:
                    pass
        return events
...
def main():
    client = CoinbaseAdvancedClient(); engine = ApexChampionShadow()
    ...
    print(f"Backfilling 72h data for {PRODUCT} Apex Champion...", flush=True)
    try:
    ...
    try:
        while True:
            try:
                ...
                print(f"  HB cash=${snap['cash']:.2f} net=${snap['realized_net']:.2f} {snap['closes']}c {snap['win_rate']}%WR {snap['pos']} Vol={snap['vol']}", flush=True)
            except Exception as e:
                runner["consecutive_exceptions"] += 1
                runner["last_exception_at"] = utc_now_iso()
                runner["last_exception_type"] = type(e).__name__
