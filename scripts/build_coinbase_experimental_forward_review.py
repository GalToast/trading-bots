...
SCOREBOARD_CSV = ROOT / "reports" / "coinbase_experimental_shadow_scoreboard.csv"
BASELINE_JSON = ROOT / "reports" / "coinbase_experimental_forward_baseline.json"
CSV_PATH = ROOT / "reports" / "coinbase_experimental_forward_review.csv"
MD_PATH = ROOT / "reports" / "coinbase_experimental_forward_review.md"
...
def seed_missing_baselines(scoreboard_rows: list[dict[str, str]], baseline: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    for row in scoreboard_rows:
        ...
        if not lane_name or lane_name == "TOTAL" or lane_name in updated:
            continue
        updated[lane_name] = {
            "seeded_at": seeded_at,
...
def classify_forward_row(
    ...
    *,
    ...
    if lane_name == "TOTAL":
        ...
        return "pack_total", str(row.get("note") or ""), float(row.get("realized_net_usd") or 0.0) - baseline_realized, 0
...
def write_reports(rows: list[dict[str, Any]], *, csv_path: Path = CSV_PATH, md_path: Path = MD_PATH) -> None:
    ...
    lines = [
        "# Coinbase Experimental Forward Review",
        "",
        "| Lane | Style | Product | Forward Status | Baseline Source | Baseline At | Baseline $ | Realized $ | Delta $ | Ratio | Baseline Closes | Closes | New Closes | Open | Cash $ | Heartbeat Age (s) | Note |",
        "| --- | --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
    for row in rows:
        ratio = row["realized_to_baseline_ratio"] if row["realized_to_baseline_ratio"] != "" else "-"
        lines.append(
            "| {lane_name} | {style} | {product_id} | {forward_status} | {baseline_source} | {baseline_at} | "
