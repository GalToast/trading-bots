#!/usr/bin/env python3
"""Momentum + dual-lattice hybrid: filter churn entries through momentum gate."""
...
def simulate_hybrid(symbol, bars, symbol_info, cfg, churn_gap, alpha, momentum_gate, cooldown_bars):
    ...
    churn_skipped_momentum = 0
    churn_skipped_cooldown = 0
    churn_entry_cooldowns: dict[float, int] = {}  # level -> bars remaining
    ...
    for idx in range(1, len(bars)):
        ...
        for direction, closed_price in closed_this_bar:
            ...
            # Cooldown check
            if cooldown_bars > 0:
                cd_key = round(closed_price, 5)
                if cd_key in churn_entry_cooldowns:
                    churn_skipped_cooldown += 1
                    continue
            # Momentum gate: bar must close in profitable direction
            if momentum_gate:
                if direction == "SELL" and bar["close"] >= closed_price:
                    churn_skipped_momentum += 1
                    continue
            ...
            if cooldown_bars > 0:
                churn_entry_cooldowns[round(closed_price, 5)] = cooldown_bars

        # Decrement cooldowns
        for key in list(churn_entry_cooldowns.keys()):
            churn_entry_cooldowns[key] -= 1
            if churn_entry_cooldowns[key] <= 0:
                del churn_entry_cooldowns[key]
    ...
    return {
        ...
        "churn_skipped_momentum": churn_skipped_momentum,
        "churn_skipped_cooldown": churn_skipped_cooldown,
...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"  Momentum + Dual-Lattice Hybrid Sweep — GBPUSD + EURUSD + NZDUSD (60d)")
    print(f"{'='*80}")

    # Cache symbol info
    sym_info = {s: mt5.symbol_info(s) for s in SYMBOLS}
    sym_bars = {s: load_bars(s, 60) for s in SYMBOLS}

    combos = [
        ("dual_lattice_baseline", False, 6, 0.0, 0),
        ("dual_lattice_alpha50", False, 6, 0.5, 0),
        ("momentum_gate_only", True, 6, 0.5, 0),
        ("cooldown_6bar", False, 6, 0.5, 6),
        ("momentum_cool6", True, 6, 0.5, 6),
        ("momentum_cool12", True, 6, 0.5, 12),
        ("cooldown_12_no_momentum", False, 6, 0.5, 12),
    ]

    all_rows = []
    for name, mom, cg, alpha, cool in combos:
        total = 0.0
        details = []
        total_skip_mom = 0
        total_skip_cool = 0
        for sym in SYMBOLS:
            ...
            r = simulate_hybrid(sym, bars, info, cfg, churn_gap=cg, alpha=alpha, momentum_gate=mom, cooldown_bars=cool)
            total += r["total"]
            total_skip_mom += r["churn_skipped_momentum"]
            total_skip_cool += r["churn_skipped_cooldown"]
        ...
        all_rows.append({"name": name, "momentum": mom, "cg": cg, "alpha": alpha, "cooldown": cool,
                         "total": total, "delta": delta, "mult": mult, "skip_mom": total_skip_mom, "skip_cool": total_skip_cool})
        print(f"\n  {name}: ${total:,.2f} (+${delta:,.2f}, {mult:.2f}x) [skipped: mom={total_skip_mom}, cool={total_skip_cool}]")
        for d in details:
            print(f"    {d}")
    ...
    for r in sorted(all_rows, key=lambda x: x["total"], reverse=True):
        print(f"  {r['name']:35s} ${r['total']:>12,.2f}  {r['mult']:.2f}x  delta=${r['delta']:>+11,.2f}")

    best = max(all_rows, key=lambda r: r["total"])
    print(f"\n🏆 Best: {best['name']} → ${best['total']:,.2f} ({best['mult']:.2f}x baseline)")

    out = ROOT / "reports" / "momentum_dual_lattice_hybrid.csv"
