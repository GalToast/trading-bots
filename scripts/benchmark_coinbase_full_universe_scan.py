#!/usr/bin/env python3
"""
Coinbase Full Universe Scanner — Phase 2
==========================================
Scan ALL remaining Coinbase coins we haven't tested yet.

Strategy: Focus on microcaps first (where RAVE's edge lives).
- Already tested: RAVE, BAL, BLUR, ALEPH, IOTX, FARTCOIN, VIRTUAL, TRUMP, FET,
  CFG, DASH, IRYS, MON, SKL, VVV, LDO, STORJ, COMP, ARB, SOL, AVAX, MATIC,
  LINK, UNI, AAVE, MKR, SNX, CRV, SUSHI, GRT, IMX, OP, APT, SUI, SEI, TIA,
  INJ, RUNE, ATOM, NEAR, FTM, ALGO, FLOW, ICP, FIL, EOS, XLM, XTZ, EGLD

- Need to test: All remaining 341 coins
- Priority: Microcaps (low price, high volatility) first
...
def main():
    ...
    print("=" * 80)
    print("  COINBASE FULL UNIVERSE SCANNER — Phase 2")
    print("=" * 80)
    ...
    for p in products:
        ...
        if pid.endswith("-USD") and not pid.endswith("-USDC") and not pid.endswith("-USDT"):
            if pid not in ALREADY_TESTED and p.get("status") == "online":
                usd_coins.append(pid)
    
    usd_coins = sorted(set(usd_coins))
    print(f"  Untested USD pairs: {len(usd_coins)}")
    
    # Prioritize microcaps: sort by base_min_size (smaller = more microcap-like)
    # Or just test alphabetically for now
    coins_to_test = usd_coins[:100]  # Test first 100 untested coins
    print(f"  Testing first {len(coins_to_test)} untested coins...")
    ...
    report = {
        ...
        "untested_usd_pairs": len(usd_coins),
    ...
    if losers:
        ...
        print(f"\n  Total loss from losers: ${total_loss:.2f}")
