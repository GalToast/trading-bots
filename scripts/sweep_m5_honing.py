#!/usr/bin/env python3
"""M5 Micro-Optimization Honing Sweep — squeeze every last drop from BTCUSD M5."""
...
def main() -> int:
    ...
    print(f"\n{'='*110}")
    print(f"  M5 MICRO-OPTIMIZATION HONING SWEEP — {SYMBOL} {DAYS}d, {len(bars)} bars")
    print(f"{'='*110}")
    
    # Dimension 1: M5 honing around $100 step sweet spot
    steps = [50, 75, 100, 125, 150]
    max_opens = [30, 35, 40, 45, 50, 60]
    gaps = [1, 2, 3]
    alphas = [0.50, 0.75, 1.00]
    momentum_gates = [True, False]
    
    # Focus on the most promising combinations first
    # Start with step=$100 variations (the known sweet spot)
    primary_sweep = []
    ...
    # Then sweep step size at optimal gap/alpha/momentum
    for step in steps:
        primary_sweep.append({
            "step": step,
            "max_open": 40,
    ...
    out_path = ROOT / "reports" / "m5_honing_sweep.csv"
