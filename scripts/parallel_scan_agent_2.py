#!/usr/bin/env python3
"""
Parallel RSI Mean-Reversion Scan — Chunk 50-99
================================================
Scans coins at lines 50-99 of coinbase_usd_pairs.txt.
...
REPORT_PATH = ROOT / "reports" / "parallel_scan_chunk_50_99.json"
...
def backtest_combo(candles, rsi_period, tp, max_hold):
    ...
    # Start scanning after RSI warmup
    for i in range(rsi_period + 1, len(candles)):
        if position is not None:
            ...
            if rsi_vals[i] <= RSI_ENTRY_THRESH:
                ...
                cash -= deploy  # cash reduces by full deploy amount
...
def main():
    print("=" * 70)
    print("  RSI Mean-Reversion Scan — Chunk 50-99")
    ...
    # Strip comments, empty lines, and summary lines like "Total: 235"
    coins = [
        l.strip() for l in lines
        if l.strip() and not l.strip().startswith("#") and not l.strip().startswith("Total:")
    ]

    slice_coins = coins[50:100]  # lines 50-99 inclusive
    print(f"\nCoins to scan: {len(slice_coins)} (indices 50-99)")
