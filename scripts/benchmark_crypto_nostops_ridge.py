...
ROWS_OUT = ROOT / "reports" / "crypto_nostops_ridge_rows.csv"
SUMMARY_OUT = ROOT / "reports" / "crypto_nostops_ridge_summary.md"

SYMBOLS = ("BTCUSD", "ETHUSD", "SOLUSD", "XRPUSD")
ALPHAS = (1.0,)
GAPS = (1,)
REARM_VARIANT_NAMES = ("rearm_lvl2_exc1", "rearm_lvl2_exc2")
MAX_OPEN_CHOICES = (30, 40, 50)
...
def main() -> int:
    ...
    try:
        ...
        lines = [
            "# Crypto No-Stops Ridge Search",
            "",
            "Focused H1 ridge search over BTCUSD, ETHUSD, SOLUSD, XRPUSD.",
            "",
            "- `alpha = 1.0`",
            "- `gap = 1`",
            "- `momentum_gate = true`",
            "- `rearm_variant in {rearm_lvl2_exc1, rearm_lvl2_exc2}`",
            "- `max_open_per_side in {30, 40, 50}`",
            "- `step_multiplier in {0.9, 1.0, 1.1, 1.25, 1.5}`",
            "",
            "## Best Per Symbol",
            "",
            "| Symbol | Combined | Realized | Floating | Closes | Avg/Close | Variant | MaxOpen | Step | StepMult |",
            "| --- | ---: | ---: | ---: | ---: | ---: | --- | ---: | ---: | ---: |",
        ]
        for symbol in SYMBOLS:
            ...
            lines.append(
                ...
                f"{best['realized_closes']} | {best['avg_close_usd']:.4f} | {best['rearm_variant']} | "
