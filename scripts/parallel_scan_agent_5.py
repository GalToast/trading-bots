#!/usr/bin/env python3
"""
Parallel RSI Mean-Reversion Scan -- Chunk 200-END
===================================================
Scans coins at lines 200-end (0-indexed) of coinbase_usd_pairs.txt.
Grid: RSI periods x TP levels x max hold = 500 combos per coin.
Entry: RSI(period) <= 30. Exit: TP hit (check high) or max_hold bars (at close).
...
REPORT_PATH = ROOT / "reports" / "parallel_scan_chunk_200_end.json"
...
def backtest_combo(candles, rsi_period, tp, max_hold):
    ...
    Entry when RSI <= 30. Exit at TP (check high) or max_hold bars (at close).
    Candle keys: "time", "close", "high".
...
def main():
    print("=" * 70)
    print("  RSI Mean-Reversion Scan -- Chunk 200-END")
    ...
    slice_coins = coins[200:]  # indices 200 to end
    print(f"\nCoins to scan: {len(slice_coins)} (indices 200-{len(coins)-1})")
