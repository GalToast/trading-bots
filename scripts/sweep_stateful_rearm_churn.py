...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sweep stateful inside-lattice re-arm variants against the current raw baseline.")
    parser.add_argument("--symbols", nargs="*", default=DEFAULT_SYMBOLS)
    parser.add_argument("--days", type=int, default=60)
    parser.add_argument(
        "--output-csv",
        default=str(ROOT / "reports" / "stateful_rearm_churn_sweep.csv"),
    )
...
def simulate_stateful_rearm(symbol: str, bars: list[dict], symbol_info, cfg: RawConfig, variant: Variant) -> dict:
    if not bars:
        return {}
...
def main() -> int:
    ...
    try:
        ...
        summary_path = out_path.with_name("stateful_rearm_churn_summary.csv")
